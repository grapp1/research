...
pdf_fig1 <- ggplot(pdf_exited_all, aes(age,Density)) + geom_line() + #scale_x_continuous(name="Age (days)",trans='log10', limits = c(100,100000), labels = scales::comma, expand=c(0,0)) +
  ...
  ggtitle("PDF of all exited particles for Scenario A (backwards tracking)") + scale_y_continuous(name=expression('Density'), expand=c(0,0), breaks = seq(0,250,25), limits = c(0,250)) +
