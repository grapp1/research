...
exit_file <- "~/Downloads/SLIM_A_v1_bw_pulse_1000ppc_exited_particles.bin"
restart_file <- "~/Downloads/SLIM_A_v1_bw_pulse_1000ppc_particle_restart.bin"
...
ggplot(exited_particles, aes(x=X, y=Z)) + geom_point() + scale_x_continuous(limits = c(0,8190), expand=c(0,0)) +
...
hist_fig <- ggplot(exited_particles, aes(age)) + geom_histogram(binwidth = 365, color = "red", fill = "red") + ggtitle("Histogram of all particles exiting the domain for Scenario A") + 
  scale_y_continuous(name="Particle Count",labels = scales::comma, expand=c(0,0),breaks = seq(0,10,2), limits = c(0,10)) + 
