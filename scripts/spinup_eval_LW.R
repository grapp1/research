...
press_files <- list.files(pattern="LW.out.press.*.pfb")
...
all_press <- array(,dim=c(nx,ny,50,limit))
...
storage[1] = storagecalc(mean(all_press[,,1,1]), 2, area, porosity)
for(j in 1:nx){
  for(k in 1:ny){
    storage_cell[j,k,1] = storagecalc(all_press[j,k,1,1], 2, cell_length**2, porosity)
  }
}

# calculating the storage for the rest of the time steps
for(i in 2:limit){
  storage[i] = storagecalc(mean(all_press[,,1,i]), 2, area, porosity)
