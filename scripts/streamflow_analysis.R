...
wbal_C <- prob_exc(wbal_C[522:1042,])
wbal_D <- prob_exc(wbal_D[522:1042,])
...
ggplot() + geom_line(data = outflow_all, aes(x = prob_exc,y = runoff_m3, group=scen,col = scen), size = 1) +
  ...
  scale_y_log10(name="Discharge (m3/hr)",limits = c(400,30000), expand=c(0,0), breaks = c(500,1000,2000,5000,10000,30000),labels = scales::comma) +
  #scale_x_reverse(trans = "log10", name="Probability of exceedance",limits = c(1,0.001), expand=c(0,0), breaks = c(seq(0.001,0.01,0.1,1))) +
  scale_x_log10(name="Probability of exceedance",limits = c(0.001,1), expand=c(0,0), breaks = c(0.001,0.01,0.1,1.0)) +
...
ggplot() + geom_point(aes(x = wbal_B$Total_surface_runoff,y = wbal_C$Total_surface_runoff)) +
  ...
  geom_abline(slope = 2, intercept = -11, col="red",linetype = "twodash") +
