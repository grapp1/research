...
exit_file_B <- "/Users/grapp/Downloads/SLIM_B_v2_bw4_exited_particles.bin"
...
exit_file_C <- "/Users/grapp/Downloads/SLIM_C_v2_bw4_exited_particles.bin"
...
exit_pts <- flowpath_fig + geom_point(data = restart_particles_1, aes(x=X, y=Y, colour = age)) + labs(color = "Age (years)") +
  scale_colour_gradient(low = "white", high="midnightblue", trans = "log",limits=c(1,50),breaks=c(5,10,20,50), 
                        labels=c("≤5","10","20","50")) +
...
#pdf_fig1 <- ggplot() + geom_line(data = pdf_exited_all, aes(x = age,y = Density_norm)) +
  ...
  scale_y_continuous(name=expression(Density~("x10"^-2)), expand=c(0,0), breaks = seq(0,0.08,0.01)*mult, limits = c(0,0.08)*mult) + 
