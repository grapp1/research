...
river_mask_df$river[river_mask_df$X == 9 & river_mask_df$Y == 23] <- 2
river_mask_df$river[river_mask_df$X == 20 & river_mask_df$Y == 40] <- 2
river_mask_df$river[river_mask_df$X == 32 & river_mask_df$Y == 19] <- 2


ggplot() + geom_tile(data = river_mask_df, aes(x = X,y = Y,fill = factor(river)), color="gray") + 
  scale_fill_manual(values=c("white","deepskyblue","black")) +
