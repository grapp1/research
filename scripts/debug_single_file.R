...
#   wt <- water_table_elev(nx,ny,1,200,-200,600,press_files[limit],paste((limit-1)*10,",000", sep=""),100,2)
  ...
  for(i in 1:20){
    for(j in 1:limit){
      ...
      press_cell[i,1] = all_press[3,20,i,j]      # saturated cell
      press_cell[i,2] = all_press[11,27,i,j]     # saturated cell
      press_cell[i,3] = all_press[18,5,i,j]    # saturated cell
      press_cell[i,4] = all_press[5,19,i,j]     # unsaturated cell
      press_cell[i,5] = all_press[25,40,i,j]     # unsaturated cell
      press_cell[i,6] = all_press[75,20,i,j]     # unsaturated cell      
    }
  }
