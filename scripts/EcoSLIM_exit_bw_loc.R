...
exit_pts <- flowpath_fig + geom_point(data = exited_particles_A_bw2, aes(x=X, y=Y, colour = age)) + labs(color = "Age (years)") +
...
# generating pdf
bin_size <- 2
pdf_exit_A_bw1 <- pdfxn(exited_particles_A_bw1, max(exited_particles_A_bw1$age), bin_size)
pdf_exit_A_bw2 <- pdfxn(exited_particles_A_bw2, max(exited_particles_A_bw2$age), bin_size)
pdf_exit_A_bw3 <- pdfxn(exited_particles_A_bw3, max(exited_particles_A_bw3$age), bin_size)
