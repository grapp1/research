...
heat_map <- function(nx,ny,layer_num,layer_thickness,lim_lo,lim_hi,press_pfb,time){
  ...
  colnames(sub_press) <- c("X","Y","pressure")
  sub_press$water_level <- sub_press$pressure+(layer_thickness/2)
  
  # plotting
  press_map <- ggplot(sub_press, aes(Y,X)) + geom_tile(aes(fill = water_level), colour = "black") + 
    scale_fill_gradient(low="blue", high="red", limits=c(lim_lo,lim_hi)) +#geom_contour(aes(z = sub_press$water_level)) + 
    #geom_text_contour(aes(z = sub_press$water_level), stroke=0.2, min.size = 10)
