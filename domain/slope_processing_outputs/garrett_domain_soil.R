...
setwd("/Users/grapp/Desktop/working/garrett/")
fundir=("/Users/grapp/Desktop/working/garrett/")
outdir=("/Users/grapp/Desktop/working/garrett/")

nodes=read.xlsx('/Users/grapp/Desktop/garrett/garrett_domain_v2.xlsx', colNames = TRUE, sheet=2)
nodes=as.data.frame(nodes)
data_wide <- melt(nodes, id=c("row","new_col"), value.name = c("elev"))
data_cast <- dcast(data_wide, row~new_col, value.var='value')
data_cast=data_cast[,-1]

write.table(data_cast, '/Users/grapp/Desktop/garrett/garrett_dem_v2.txt', sep = " ", col.names = FALSE, row.names = FALSE, qmethod = "double")
...
scale=0.001 #The maximum ratio of secondary to primary flow directions (set to -1 if you don't want the secondary slopes to be scaled, set to 0 if you want only primary flow directios)
...
image.plot((temp*2), add=T, col=maskcol(2), legend=FALSE)
...
image.plot(area)
#write.table(area, file="~/research/domain/trib_area_cell", sep="\t", row.names=FALSE, col.names=FALSE)
...
outdir=("~/research/domain/slope_processing_outputs/")
rivermask=rivers #This should be the river mask you input to the slope processing. could also be 'rivers' if you calcualted your own river mask for that
